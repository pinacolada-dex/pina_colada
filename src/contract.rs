...
use cosmwasm_std::{
    ...
    Response, StdError, StdResult, SubMsgResponse, SubMsgResult,
};
...
use crate::handlers::{
    execute_create_pair, execute_modify_position, execute_provide_liquidity,
    execute_swap_operations, execute_withdraw_liquidity, generate_key_from_asset_info, DENOM,
};

use crate::query::{query_compute_d, query_config, query_lp_price, simulate_swap_operations};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::ExecuteSwapOperations {
            ...
            assert_eq!(
                operations[0].clone().offer_asset_info,
        ...
        ExecuteMsg::CreatePair {
            ...
            token_code_id: _,
            init_params,
        } => execute_create_pair(&mut deps, env, info, init_params, asset_infos),
    ...
    match from_json(&cw20_msg.msg)? {
        Cw20HookMsg::ExecuteSwapOperations {
            ...
            let pool_key = generate_key_from_asset_info(
                [
                    operations[0].clone().offer_asset_info,
                    operations[0].clone().ask_asset_info,
                ]
                .as_ref(),
            );
        ...
        Cw20HookMsg::WithdrawLiquidity { assets } => execute_withdraw_liquidity(
            deps,
            env,
            info.clone(),
            info.sender.clone(),
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> Result<Binary, ContractError> {
    match msg {
        ...
        QueryMsg::Pair { pool_key } => Ok(to_json_binary(
            &POOLS.load(deps.storage, pool_key)?.pair_info,
        )?),
        QueryMsg::ComputeD { pool_key } => {
            Ok(to_json_binary(&query_compute_d(deps, env, pool_key)?)?)
        }
        QueryMsg::Config { pool_key } => Ok(to_json_binary(&query_config(deps, env, pool_key)?)?),
        QueryMsg::LpPrice { pool_key } => {
            Ok(to_json_binary(&query_lp_price(deps, env, pool_key)?)?)
        }
    }
}
